...
func (p *Pipeline) SetPipelineSonar(play ci.Play, r *PlayReconciler) error {
	log := r.Log.WithName("SetPipelineSonar").WithValues(" cx-namespace", InNamespace(play))
	...
	task := tkn.PipelineTask{
		...
		TaskRef: &tkn.TaskRef{
			...
			Name: CxCINamespacedName(ci.Sonar.String(), play).Name,
		},
		Workspaces: wks,
		RunAfter:   p.RunAfter,
	}
